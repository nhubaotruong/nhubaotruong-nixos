{ config, lib, pkgs, ... }: {
  ...
  config = lib.mkIf config.environment.fhs.enable {
    systemd.tmpfiles.rules = lib.optionals config.environment.fhs.linkExes [
      ...
      "L+ /lib     - - - - /run/current-system/sw/lib"
      "L+ /lib64   - - - - /run/current-system/sw/lib"
      "L+ /usr/lib - - - - /run/current-system/sw/lib"
    ]);
    environment.sessionVariables.LD_LIBRARY_PATH =
      lib.mkIf config.environment.fhs.linkLibs (lib.mkForce "/lib");
    system.activationScripts.binsh =
      lib.mkIf config.environment.fhs.linkExes (lib.mkForce "");
